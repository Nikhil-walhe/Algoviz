...
const AVLTreeVisualizer = () => {
  ...
  const [message, setMessage] = useState('Enter numbers to build your AVL tree');
  const [balanceOperations, setBalanceOperations] = useState([]);
  ...
  const config = {
    ...
    padding: 40,
    balanceFactorFontSize: 10,
    balanceFactorOffset: 20
  };
  ...
  const rotateRight = (y) => {
    ...
    setBalanceOperations(prev => [...prev, `Rotate Right at ${y.value}`]);
    return x;
  };
  ...
  const rotateLeft = (x) => {
    ...
    setBalanceOperations(prev => [...prev, `Rotate Left at ${x.value}`]);
    return y;
  };
  ...
  const insert = async (value) => {
    ...
    const newTree = insertHelper(tree, num);
    setTree({...newTree}); // Force re-render
    ...
    setMessage(`Added node with value ${num}. ${balanceOperations.length > 0 ? 
      `Performed ${balanceOperations.join(', ')} to balance tree.` : 'Tree remained balanced.'}`);
  };
      ...
      return (
        <g key={node.id}>
          ...
          >
            ...
            <text
              x={node.x}
              y={node.y + config.balanceFactorOffset}
              textAnchor="middle"
              fill="#10b981"
              fontSize={config.balanceFactorFontSize}
            >
  ...
  const clearTree = () => {
    ...
    setActiveNode(null);
    setBalanceOperations([]);
    setMessage('Tree cleared. Enter numbers to build a new AVL tree.');
  };
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-4 md:p-6 flex flex-col">
      <div className="max-w-7xl w-full mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-yellow-400 mb-2">AVL Tree Visualizer</h1>
            <p className="text-yellow-200">Self-balancing binary search tree</p>
          </div>
          <Link href="/tree">
            <motion.button
              whileHover={{ scale: 1.05, backgroundColor: "#a16207" }}
              whileTap={{ scale: 0.95 }}
              className="px-6 py-2 bg-yellow-700 text-white rounded-lg shadow-lg hover:bg-yellow-600 transition-all flex items-center gap-2"
            >
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-1/4 flex flex-col gap-6">
            ...
            {balanceOperations.length > 0 && (
              <div className="bg-gray-800 rounded-xl p-6 shadow-lg">
                <h3 className="text-yellow-300 text-lg mb-2">Balance Operations</h3>
                <div className="flex flex-wrap gap-2">
                  {balanceOperations.map((op, index) => (
                    <motion.div
                      key={index}
                      initial={{ opacity: 0, scale: 0.8 }}
                      animate={{ opacity: 1, scale: 1 }}
                      className="bg-yellow-900 text-yellow-100 px-3 py-1 rounded-full text-sm"
                    >
                      {op}
                    </motion.div>
                  ))}
