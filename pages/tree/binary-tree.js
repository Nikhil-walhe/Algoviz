...
const BinaryTreeVisualizer = () => {
  ...
  const [message, setMessage] = useState('Enter numbers to build your binary tree');
  ...
  const insert = async (value) => {
    ...
    const insertNode = (node, value, parent = null, direction = null) => {
      if (!node) {
        const newNode = new TreeNode(value);
        path.push({ node: newNode, parent, direction });
        return newNode;
      }

      path.push({ node, parent, direction });

      if (value < node.value) {
        node.left = insertNode(node.left, value, node, 'left');
      } else if (value > node.value) {
        node.right = insertNode(node.right, value, node, 'right');
      }

      return node;
    };

    const updatedTree = insertNode(newTree, num);
    setTree(updatedTree);
  ...
  const clearTree = () => {
    ...
    setTraversalResult([]);
    setActiveNode(null);
    setMessage('Tree cleared. Enter numbers to build a new tree.');
  };

  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 p-4 md:p-6 flex flex-col">
      <div className="max-w-7xl w-full mx-auto">
        <h1 className="text-3xl md:text-4xl font-bold text-yellow-400 mb-2">Binary Tree Visualizer</h1>
        <p className="text-yellow-200 mb-6">Automatically scaled and centered</p>
